...
#warning("** Instantiating Kalman filter \"" STRINGIFY(KALMAN_NAME) "\" with " STRINGIFY(KALMAN_NUM_STATES) " states and " STRINGIFY(KALMAN_NUM_INPUTS) " inputs")
...
#warning("Creating Kalman filter A buffer: " STRINGIFY(__KALMAN_BUFFER_A))
static matrix_data_t __KALMAN_BUFFER_A[__KALMAN_A_ROWS * __KALMAN_A_COLS];

#warning("Creating Kalman filter P buffer: " STRINGIFY(__KALMAN_BUFFER_P))
static matrix_data_t __KALMAN_BUFFER_P[__KALMAN_P_ROWS * __KALMAN_P_COLS];

#warning("Creating Kalman filter x buffer: " STRINGIFY(__KALMAN_BUFFER_x))
...
#warning("Creating Kalman filter B buffer: " STRINGIFY(__KALMAN_BUFFER_B))
static matrix_data_t __KALMAN_BUFFER_B[__KALMAN_B_ROWS * __KALMAN_B_COLS];

#warning("Creating Kalman filter Q buffer: " STRINGIFY(__KALMAN_BUFFER_Q))
static matrix_data_t __KALMAN_BUFFER_Q[__KALMAN_Q_ROWS * __KALMAN_Q_COLS];

#warning("Creating Kalman filter u buffer: " STRINGIFY(__KALMAN_BUFFER_u))
static matrix_data_t __KALMAN_BUFFER_u[__KALMAN_x_ROWS * __KALMAN_u_COLS];

#else

#warning("Skipping Kalman filter B buffer: (zero inputs)")
#define __KALMAN_BUFFER_B ((matrix_data_t*)0)

#warning("Skipping Kalman filter Q buffer: (zero inputs)")
#define __KALMAN_BUFFER_Q ((matrix_data_t*)0)

#warning("Skipping Kalman filter u buffer: (zero inputs)")
...
#warning("Creating Kalman filter aux buffer: " STRINGIFY(__KALMAN_BUFFER_aux))
static matrix_data_t __KALMAN_BUFFER_aux[__KALMAN_aux_size];

#warning("Creating Kalman filter temporary P/BQ buffer: " STRINGIFY(__KALMAN_BUFFER_tempPBQ))
...
#warning("Creating Kalman filter structure: " STRINGIFY(KALMAN_STRUCT_NAME))
...
#warning ("Creating Kalman filter initialization function: " STRINGIFY(KALMAN_FUNCTION_NAME(init()) ))
